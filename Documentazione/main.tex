...
\begin{itemize}
\item \textbf{CBOW : } usa un vettore di parole per predire un contesto target
\item \textbf{Skim Gram : }  è denominato metodo “Continuous Bag of Words” (CBOW), per predire una parola target basata su una sequenza contigua di n-grammi.
\end{itemize}

: quello esposto — dove un vettore di parole è usato per predire un contesto target — che è detto metodo skip-gram; il secondo invece è denominato metodo “Continuous Bag of Words” (CBOW), il quale utilizza il contesto per predire una parola target basata su una sequenza contigua di n-grammi.
